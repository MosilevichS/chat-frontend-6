...
const ChatsItem = ({ chat, selectedChatId, handleRightClick }: ChatItemProps) => {
  ...
  // const getLastMessageTime = (): number | null => {
  //   return chat.last_message?.created_at || null;
  // };

  // const isMessageNew = (): boolean => {
  //   return !!chat.last_message?.new;
  // };

  const getNewMessageCount = (): number => {
  ...
  return (
    <div className={`h-[80px] px-2 py-1 ${isFavorite() ? "bg-white" : ""}`}>
      <Link
        href={chatPath}
        ...
        className={`
          ...
          ${isActive ? "bg-(--color-violet-dark-opacity) hover:bg-(--color-violet-dark-opacity)" : ""}
        `}
      >
